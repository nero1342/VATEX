...
from vatex.utils.misc import is_dist_avail_and_initialized
...
class VideoSetCriterion(nn.Module):
    ...
    def loss_contrastive(self, outputs):
        ...
        if dist.get_world_size() > 1:
            sentence_feats = torch.cat(GatherLayer.apply(sentence_feats), dim=0)
        
        query = sentence_feats[0::3]
        negative_key = sentence_feats[1::3]
        positive_key = sentence_feats[2::3]

        loss = InfoNCE(negative_mode='unpaired')
        l = loss(query, positive_key, negative_key) + loss(positive_key, query, negative_key)
    ...
    def forward(self, outputs, targets):
        ...
        # Contrastive 
        losses.update(self.loss_contrastive(outputs))
