...
Registry for transformer module in VATEX.
"""

@TRANSFORMER_DECODER_REGISTRY.register()
class VATEXTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, caption, list_vis, mask = None, texts = None):
        text_word_features, text_word_masks, text_pos, text_sentence_features = caption
        ...
        output = text_sentence_features.unsqueeze(0).repeat(query_embed.size(0), 1, 1)
        ...
        if list_vis is not None:
            # print(output.shape, list_vis.shape)
            output = output + list_vis
        ...
        out = {
            ...
            'sentence_feats': text_sentence_features,
            "texts": texts,
        }
        return out
