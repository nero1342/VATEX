...
class VideoSwinTransformerBackbone(Backbone):
    """
    ...
    def __init__(self, cfg, input_shape):
        ...
        backbone_pretrained = cfg.MODEL.VIDEO_SWIN.PRETRAINED
        backbone_pretrained_path = cfg.MODEL.VIDEO_SWIN.PRETRAINED_PATH
        train_backbone = cfg.MODEL.VIDEO_SWIN.TRAIN_BACKBONE
        version = cfg.MODEL.VIDEO_SWIN.VERSION 
        assert version in ['video_swin_t_p4w7', 'video_swin_s_p4w7', 'video_swin_b_p4w7']
        version_cfgs = configs[version]
        # out_indices = (0, 1, 2, 3)
        self.num_layers = len(version_cfgs["depths"])
        self.num_features = [int(version_cfgs["embed_dim"] * 2 ** i) for i in range(self.num_layers)]
        ...
        swin_backbone = SwinTransformer3D(**version_cfgs)
        # if backbone_pretrained and isinstance(backbone_pretrained_path, str) and backbone_pretrained_path != "":
    ...
    def forward(self, samples: torch.Tensor, num_frames = 2):
        ...
        for idx, (layer, downsample) in enumerate(zip(self.layers, self.downsamples)):
            ...
            out[f"res{str(idx + 2)}"] = vid_embeds
            if downsample:
